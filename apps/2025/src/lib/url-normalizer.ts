...
const socialPlatforms: Record<string, SocialPlatform> = {
  ...
  github: {
    ...
    patterns: [/(?:https?:\/\/)?(?:www\.)?github\.com\/([^\/\?&#]+)/i, /^([a-zA-Z0-9-]+)$/],
    normalizer: (input: string) => {
 ...
 */
export function normalizeSocialUrl(platform: keyof typeof socialPlatforms, input: string): string {
 ...
 */
export function normalizeAllSocialUrls<T extends Record<string, unknown>>(data: T): T {
